...
exports.upgradeUserToPremium = async (req, res) => {
  try {
    ...
    if (!updatedUser) {
      return res
        .status(404)
        .json({ message: "Could not find or create user in our database." });
    }
    res.status(200).json({
    ...
    res
      .status(500)
      .json({ message: "Server error during upgrade.", error: error.message });
  }
};
...
exports.saveRight = async (req, res) => {
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: "You have already saved this right." });
    }
    ...
    res.status(500).json({
      message: "Server error while saving right.",
      error: error.message,
    });
  }
};
...
exports.getSavedRights = async (req, res) => {
  try {
    ...
    const savedRights = await SavedRight.find({ userId })
      .populate("scenarioId")
      .sort({ savedAt: -1 });
    res.status(200).json(savedRights);
  } catch (error) {
    res.status(500).json({
      message: "Server error while fetching saved rights.",
      error: error.message,
    });
  }
};
