document.addEventListener("DOMContentLoaded", () => {
  ...
  const elements = {
    ...
    micBtn: document.getElementById("mic-btn"), // New: Contact Form elements
...
        `;
  ...
  const handleMenuToggle = () => {
    const isExpanded =
      elements.menuToggleBtn.getAttribute("aria-expanded") === "true";
  ...
  const handleShareResult = async () => {
    const resultsContainer = document.getElementById("results-content");
    const textToShare = resultsContainer?.innerText; // Check if the Web Share API is available in the browser
    ...
    if (navigator.share && textToShare) {
      ...
      alert(
        "Web Share is not supported by your browser, or there is no content to share."
      );
    }
  };
  ...
  const handleResultsClick = async (e) => {
    ...
    if (explainBtn) {
      ...
      explainBtn.querySelector("span").textContent =
        constants.BUTTON_TEXT.EXPLAIN_LOADING;
  ...
  if (elements.contactForm) {
    elements.contactForm.addEventListener("submit", function (event) {
      ...
      submitButton.disabled = true; // Your EmailJS IDs
